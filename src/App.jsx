...
function GradientDivider() {
  return <div className="h-[2px] w-24 mx-auto my-4" style={{ background: 'linear-gradient(90deg, #00f0ff, #ff2d95, #7b2dff)' }} />
}
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center px-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full opacity-20" style={{ background: 'radial-gradient(circle, #ff2d95 0%, transparent 70%)' }} />
      </div>
      <div className="text-center max-w-md relative z-10">
        <h1 className="font-display text-6xl md:text-7xl tracking-wider mb-2 gradient-text">VENDETTA</h1>
        <GradientDivider />
        <p className="text-text-secondary mt-6 mb-8 leading-relaxed">This site contains age-restricted products including nicotine. You must be 21 or older to enter.</p>
        <p className="font-display text-3xl md:text-4xl tracking-wide mb-8">ARE YOU 21+?</p>
        <div className="flex gap-4 justify-center">
          <button onClick={onVerified} className="btn-shimmer px-10 py-3.5 rounded font-semibold text-white transition-all duration-300 hover:scale-105 hover:shadow-[0_0_40px_rgba(0,240,255,0.5)]" style={{ background: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>YES, ENTER</button>
          <button onClick={() => { window.location.href = 'https://google.com' }} className="px-10 py-3.5 rounded font-semibold border border-white/20 text-white hover:border-white/60 transition-all duration-300">NO, EXIT</button>
        </div>
      </div>
    </div>
  )
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass shadow-lg shadow-black/30' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between px-5 py-4">
        ...
        <div className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((l) => (<a key={l.href} href={l.href} className="text-sm text-text-secondary hover:text-accent-cyan transition-colors duration-300">{l.label}</a>))}
          <a href="#for-venues" className="btn-shimmer px-6 py-2.5 rounded text-sm font-semibold text-white transition-all duration-300 hover:scale-105 hover:shadow-[0_0_25px_rgba(0,240,255,0.4)]" style={{ background: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>PARTNER WITH US</a>
        </div>
        <button className="md:hidden text-white" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={24} /> : <Menu size={24} />}</button>
      </div>
      {menuOpen && (
        <div className="md:hidden glass border-t border-white/5 px-5 pb-6 pt-2 space-y-4">
          {NAV_LINKS.map((l) => (<a key={l.href} href={l.href} onClick={close} className="block text-text-secondary hover:text-accent-cyan transition-colors">{l.label}</a>))}
          <a href="#for-venues" onClick={close} className="inline-block px-5 py-2 rounded text-sm font-semibold text-white" style={{ background: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>PARTNER WITH US</a>
        </div>
      )}
    </nav>
  )
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden px-5 scan-overlay">
      ...
      <style>{`
        ...
        .orb-1 { width: 700px; height: 700px; background: radial-gradient(circle, #00f0ff 0%, transparent 70%); top: -20%; left: -15%; opacity: 0.3; animation: drift1 14s ease-in-out infinite alternate; }
        .orb-2 { width: 600px; height: 600px; background: radial-gradient(circle, #ff2d95 0%, transparent 70%); bottom: -20%; right: -15%; opacity: 0.35; animation: drift2 16s ease-in-out infinite alternate; }
        .orb-3 { width: 400px; height: 400px; background: radial-gradient(circle, #7b2dff 0%, transparent 70%); top: 30%; left: 60%; opacity: 0.25; animation: drift3 12s ease-in-out infinite alternate; }
        .orb-4 { width: 300px; height: 300px; background: radial-gradient(circle, #ffb800 0%, transparent 70%); bottom: 30%; left: 20%; opacity: 0.15; animation: drift4 18s ease-in-out infinite alternate; }
      ...
      <div className="relative z-10 text-center max-w-4xl">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-accent-cyan/30 bg-accent-cyan/5 mb-8">
          ...
          <span className="text-accent-cyan text-sm font-medium tracking-wide">LIVE ACROSS NJ & NY NIGHTLIFE</span>
        </div>
        <h1 className="font-display text-5xl sm:text-6xl md:text-7xl lg:text-9xl tracking-wide leading-[0.9] mb-6">
          <span className="gradient-text">YOUR VENUE.</span><br />
          <span className="text-white">OUR MACHINES.</span><br />
          <span className="gradient-text">PURE PROFIT.</span>
        </h1>
        <p className="text-text-secondary text-lg md:text-xl max-w-2xl mx-auto mb-12 leading-relaxed">
          The dominant vape vending machine network across New Jersey and New York. Premium product. Premium placement. Zero cost to you.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
          <a href="#for-venues" className="btn-shimmer px-10 py-4 rounded font-semibold text-white text-lg transition-all duration-300 hover:scale-105 hover:shadow-[0_0_50px_rgba(0,240,255,0.4)]" style={{ background: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>GET A MACHINE IN YOUR VENUE</a>
          <a href="#products" className="group px-10 py-4 rounded font-semibold border border-white/20 text-white text-lg hover:border-accent-cyan hover:text-accent-cyan transition-all duration-300 flex items-center justify-center gap-2">SEE WHAT WE STOCK <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" /></a>
        </div>
        <div className="grid grid-cols-3 gap-6 max-w-xl mx-auto">
          <div>
            <div className="font-display text-3xl md:text-4xl stat-number">50+</div>
            <div className="text-text-muted text-xs uppercase tracking-wider mt-1">Venues Partnered</div>
          </div>
          <div>
          ...
          <div>
            <div className="font-display text-3xl md:text-4xl stat-number">$8K+</div>
            <div className="text-text-muted text-xs uppercase tracking-wider mt-1">Avg Monthly / Machine</div>
          </div>
        </div>
      </div>
    </section>
...
function StepCard({ s }) {
  const ref = useScrollReveal()
  return (
    <div ref={ref} className="reveal text-center glow-card bg-bg-secondary rounded-2xl p-10 border border-white/5 group hover:bg-bg-tertiary transition-all duration-500">
      <span className="font-display text-6xl bg-clip-text text-transparent block mb-4" style={{ backgroundImage: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>{s.num}</span>
...
function ProductCard({ p }) {
  const ref = useScrollReveal()
  return (
    <div ref={ref} className="reveal glow-card bg-bg-secondary rounded-2xl p-7 border border-white/5 group hover:bg-bg-tertiary transition-all duration-500">
...
function ValuePropCard({ v }) {
  const ref = useScrollReveal()
  return (
    <div ref={ref} className="reveal glow-card bg-bg-secondary rounded-2xl p-8 border border-white/5 hover:bg-bg-tertiary transition-all duration-500">
...
function ForVenues() {
  ...
  return (
    <section id="for-venues" className="relative z-10 py-28 px-5">
      <SectionHeading sub="Your competitors already have one. Do you?">Partner With Us</SectionHeading>
      <div className="max-w-5xl mx-auto grid sm:grid-cols-2 gap-6 mb-20">
        {VALUE_PROPS.map((v) => <ValuePropCard key={v.title} v={v} />)}
      </div>
      <div className="max-w-xl mx-auto">
        <form action="https://formsubmit.co/contact@getvendetta.com" method="POST" className="glow-card bg-bg-secondary border border-white/5 rounded-2xl p-10 space-y-5">
          <input type="hidden" name="_subject" value="New Vendetta Venue Partnership Inquiry" />
          <input type="hidden" name="_captcha" value="false" />
          <input type="hidden" name="_template" value="table" />
          <h3 className="font-display text-3xl tracking-wide text-center mb-6">Get Started Today</h3>
          <input required name="Venue Name" placeholder="Venue Name" className={inputCls} />
          <input required name="Contact Name" placeholder="Contact Name" className={inputCls} />
          <div className="grid sm:grid-cols-2 gap-4">
            <input required type="email" name="Email" placeholder="Email" className={inputCls} />
            <input required type="tel" name="Phone" placeholder="Phone" className={inputCls} />
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <select required name="Venue Type" defaultValue="" className={selectCls}>
              <option value="" disabled>Venue Type</option>
              {VENUE_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
            <select required name="Weekend Traffic" defaultValue="" className={selectCls}>
              <option value="" disabled>Est. Weekend Nightly Traffic</option>
              {TRAFFIC_OPTIONS.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <textarea name="Message" placeholder="Message / Questions (optional)" rows={3} className={inputCls} />
          <button type="submit" className="btn-shimmer w-full py-4 rounded-lg font-semibold text-white text-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_40px_rgba(0,240,255,0.35)]" style={{ background: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>LET'S TALK</button>
        </form>
      </div>
    </section>
  )
...
function FomoStatCard({ value, label }) {
  const ref = useScrollReveal()
  return (
    <div ref={ref} className="reveal text-center glow-card bg-bg-secondary rounded-2xl p-8 border border-white/5">
      <div className="font-display text-4xl md:text-5xl stat-number mb-2">{value}</div>
...
function NumbersSection() {
  return (
    <section className="relative z-10 py-28 px-5">
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-accent-pink/[0.04] to-transparent pointer-events-none" />
      <SectionHeading sub="Our partners are cashing in every single night. Are you?">The Numbers Don't Lie</SectionHeading>
      <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 relative z-10 mb-12">
        <FomoStatCard value="$2,400+" label="Avg Monthly Venue Commission" />
        <FomoStatCard value="35%" label="Longer Guest Stay Time" />
        <FomoStatCard value="89%" label="Partner Retention Rate" />
        <FomoStatCard value="$0" label="Upfront Cost" />
      </div>
      <div className="text-center relative z-10">
        <p className="font-display text-2xl md:text-3xl tracking-wide gradient-text max-w-3xl mx-auto mb-8">Every night without a Vendetta machine is revenue walking out your door.</p>
        <a href="#for-venues" className="btn-shimmer inline-block px-10 py-4 rounded font-semibold text-white text-lg transition-all duration-300 hover:scale-105 hover:shadow-[0_0_50px_rgba(255,45,149,0.4)]" style={{ background: 'linear-gradient(135deg, #ff2d95, #7b2dff)' }}>STOP LEAVING MONEY ON THE TABLE</a>
      </div>
    </section>
  )
}
      ...
      <div ref={ref} className="reveal max-w-4xl mx-auto">
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          <div className="glow-card bg-bg-secondary rounded-2xl p-8 border border-white/5">
            <div className="flex items-center gap-3 mb-6"><MapPin size={24} className="text-accent-cyan" /><h3 className="font-display text-2xl tracking-wide">NEW JERSEY</h3></div>
            <div className="grid grid-cols-2 gap-3">
              {NJ_AREAS.map((area) => (<div key={area} className="flex items-center gap-2"><span className="pulse-dot" style={{ width: 6, height: 6 }} /><span className="text-text-secondary text-sm">{area}</span></div>))}
            </div>
          </div>
          <div className="glow-card bg-bg-secondary rounded-2xl p-8 border border-white/5">
            <div className="flex items-center gap-3 mb-6"><MapPin size={24} className="text-accent-pink" /><h3 className="font-display text-2xl tracking-wide">NEW YORK</h3></div>
            <div className="grid grid-cols-2 gap-3">
              {NY_AREAS.map((area) => (<div key={area} className="flex items-center gap-2"><span className="pulse-dot" style={{ width: 6, height: 6 }} /><span className="text-text-secondary text-sm">{area}</span></div>))}
            </div>
          </div>
        </div>
        <div className="text-center">
          <p className="text-text-secondary mb-6">Don't see your area? We're expanding fast.</p>
          <a href="#for-venues" className="btn-shimmer inline-block px-8 py-3.5 rounded-lg font-semibold text-white transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(0,240,255,0.4)]" style={{ background: 'linear-gradient(135deg, #00f0ff, #ff2d95)' }}>REQUEST YOUR AREA</a>
        </div>
      </div>
    </section>
  )
...
function FaqItem({ f, isOpen, onToggle }) {
  const ref = useScrollReveal()
  return (
    <div ref={ref} className="reveal border border-white/5 rounded-xl overflow-hidden">
      <button type="button" onClick={onToggle} className="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-white/[0.02] transition-colors">
        <span className="font-semibold pr-4">{f.q}</span>
        {isOpen ? <ChevronUp size={18} className="flex-shrink-0 text-accent-cyan" /> : <ChevronDown size={18} className="flex-shrink-0 text-text-muted" />}
      </button>
      <div className="transition-all duration-300 overflow-hidden" style={{ maxHeight: isOpen ? '300px' : '0' }}>
  ...
  return (
    <section id="faq" className="relative z-10 py-28 px-5">
      ...
      <div className="max-w-2xl mx-auto space-y-3 relative z-10">
        {FAQS.map((f, i) => <FaqItem key={i} f={f} isOpen={openIndex === i} onToggle={() => setOpenIndex(openIndex === i ? null : i)} />)}
      </div>
    </section>
  )
}
        ...
        <div className="grid md:grid-cols-3 gap-12 mb-14">
          <div>
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-accent-cyan/20 bg-accent-cyan/5">
              ...
              <span className="text-accent-cyan text-xs font-medium">50+ Venues Partnered</span>
            </div>
          </div>
          <div>
            <h4 className="font-semibold mb-4 text-sm uppercase tracking-wider text-text-muted">Navigation</h4>
            <div className="space-y-3">
              {NAV_LINKS.map((l) => (<a key={l.href} href={l.href} className="block text-text-secondary text-sm hover:text-accent-cyan transition-colors duration-300">{l.label}</a>))}
            </div>
          </div>
          <div>
            <h4 className="font-semibold mb-4 text-sm uppercase tracking-wider text-text-muted">Contact</h4>
            <div className="space-y-3 text-sm text-text-secondary">
              <a href="mailto:contact@getvendetta.com" className="flex items-center gap-2 hover:text-accent-cyan transition-colors duration-300"><Mail size={16} /> contact@getvendetta.com</a>
              <a href="tel:+12018005506" className="flex items-center gap-2 hover:text-accent-cyan transition-colors duration-300"><Phone size={16} /> (201) 800-5506</a>
              <div className="flex items-center gap-2"><MapPin size={16} /> New Jersey & New York</div>
            </div>
            <div className="flex gap-4 mt-5">
              <a href="#" className="text-text-muted hover:text-accent-cyan transition-colors duration-300"><Instagram size={20} /></a>
              <a href="#" className="text-text-muted hover:text-accent-cyan transition-colors duration-300"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5" /></svg></a>
...
export default function App() {
  ...
  useEffect(() => {
    document.body.style.overflow = ageVerified ? '' : 'hidden'
